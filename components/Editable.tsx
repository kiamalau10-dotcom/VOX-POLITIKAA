...
const Editable: React.FC<EditableProps> = ({ cmsKey, children, className = '', type = 'text' }) => {
  ...
  const toggleColor = () => {
    const colors = ['', 'text-slate-900', 'text-blue-600', 'text-green-600', 'text-yellow-600', 'text-purple-600'];
  ...
  return (
    <MotionTag
      ...
      style={{ x: position.x, y: position.y }}
      className={`relative group border-2 border-dashed ${isDragging ? 'border-slate-500 z-50' : 'border-transparent hover:border-slate-400/50'} transition-colors ${className} ${color} ${type === 'text' ? 'inline-block' : ''}`}
    >
      {isEditMode && (
        <span className="absolute -top-8 left-0 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-50 bg-white dark:bg-zinc-900 p-1 rounded-lg shadow-xl border border-slate-500/20">
          <button onClick={toggleColor} className="p-1 hover:bg-slate-500/10 rounded text-slate-900"><Palette size={14} /></button>
      ...
      {type === 'text' ? (
        <span
          ...
          className="outline-none focus:ring-2 focus:ring-slate-500/20 rounded px-1 inline-block min-w-[1ch]"
        >
          {content}
        </span>
      ) : (
