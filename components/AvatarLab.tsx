...
const AvatarLab: React.FC<AvatarLabProps> = ({ currentUser, onUpdateUser, onClose }) => {
  ...
  const calculateTotalCost = () => {
    ...
    Object.entries(AVATAR_OPTIONS).forEach(([category, options]) => {
      const selectedId = config[category as keyof typeof AVATAR_OPTIONS];
      const option = (options as any).find((o: any) => o.id === selectedId);
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-6">
      ...
      <motion.div 
        ...
        className="relative w-full max-w-6xl h-[85vh] rounded-[3rem] border overflow-hidden flex flex-col md:flex-row bg-white border-black/5 shadow-2xl"
      >
        {/* 2D Preview Area */}
        <div className="flex-1 relative border-r flex items-center justify-center bg-zinc-50 border-black/5">
          <Avatar2D username={currentUser.username} config={config} />

          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 text-center pointer-events-none">
            <h2 className="text-3xl font-black uppercase italic text-slate-900 tracking-tighter">Avatar Lab 2.0</h2>
            <p className="text-[10px] font-bold opacity-50 uppercase tracking-[0.3em] mt-1 text-slate-900">2D Identity System</p>
            {voxTitle !== 'none' && (
              <motion.p 
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className="mt-4 text-xs font-black bg-slate-900 text-white px-4 py-1 rounded-full uppercase tracking-widest shadow-lg"
              >
          ...
          <div className="absolute top-8 left-8 flex flex-col gap-2">
            <div className="flex items-center gap-2 bg-slate-900 text-white px-4 py-2 rounded-full shadow-lg shadow-black/20">
        ...
        <div className="flex-1 flex flex-col h-full bg-white text-slate-900">
                      ...
                      return (
                        <button
                          ...
                          className={`p-3 rounded-xl border-2 transition-all flex flex-col items-center gap-1 ${
                            config[category as keyof typeof config] === opt.id
                              ? 'border-slate-900 bg-slate-900/5'
                              : 'border-black/5 bg-zinc-50 hover:bg-zinc-100'
                          }`}
                        >
                          <span className="text-[10px] font-bold uppercase text-slate-900">{opt.label}</span>
                          {!owned && opt.price > 0 && (
                            <span className="text-[8px] text-slate-950 font-black">🪙{opt.price}</span>
                          )}
                          {owned && opt.price > 0 && (
                            <span className="text-[8px] text-green-500 font-black uppercase">Owned</span>
                          )}
                          {config[category as keyof typeof config] === opt.id && <Check size={10} className="text-slate-900" />}
                        </button>
                      );
                    })}
                  </div>
                    ...
                    <button
                      ...
                      className={`p-6 rounded-[2rem] border-2 transition-all flex flex-col items-center gap-3 text-center ${
                        equippedCostumeId === costume.id
                          ...
                          : 'border-black/5 bg-zinc-50 hover:border-black/20 hover:bg-zinc-100'
                      }`}
                    >
                      <div className="w-12 h-12 bg-slate-900/10 rounded-2xl flex items-center justify-center text-2xl shadow-sm">
                        {costume.icon}
                      </div>
                      <div>
                        <p className="text-xs font-black uppercase mb-1 text-slate-900">{costume.label}</p>
                        {!owned && costume.price > 0 && (
                          <p className="text-[10px] font-black text-slate-900">🪙 {costume.price}</p>
                        )}
                        {owned && costume.price > 0 && (
                      ...
                      {equippedCostumeId === costume.id ? (
                        <span className="text-[8px] font-black bg-slate-900 text-white px-2 py-1 rounded-full uppercase shadow-sm">Equipped</span>
                      ) : (
                        <span className="text-[8px] font-black opacity-30 uppercase text-slate-500">Select</span>
                      )}
                    </button>
                  );
                })}
                ...
                {VOX_TITLES.map((title) => (
                  <button
                    ...
                    className={`p-4 rounded-xl border-2 transition-all text-left ${
                      voxTitle === title.id
                        ? 'border-red-600 bg-red-600/5 shadow-inner'
                        : 'border-black/5 bg-zinc-50 hover:bg-zinc-100'
                    }`}
                  >
                    <p className="text-[10px] font-black uppercase tracking-wider text-slate-900">{title.label}</p>
                    {voxTitle === title.id && <Check size={10} className="text-slate-900 mt-1" />}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="p-8 border-t border-black/5 bg-white">
            <button 
              onClick={handleSave}
              className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-black transition-all active:scale-95 shadow-2xl shadow-black/30"
            >
