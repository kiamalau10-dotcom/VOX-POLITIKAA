...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center transition-colors duration-300 bg-transparent">
      <motion.div 
        initial={{ opacity: 0, y: 20 }} 
        animate={{ opacity: 1, y: 0 }} 
        className="p-10 rounded-3xl shadow-2xl w-full max-w-md border bg-white/90 border-black/5"
      >
        <h2 className="text-3xl font-black italic text-center mb-2 tracking-tighter uppercase text-slate-900">
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          {isSignUpMode && (
            <input 
              ...
              className="w-full p-4 rounded-xl border-2 outline-none transition-all bg-white border-slate-200 focus:border-slate-900 text-slate-900"
          ...
          <div className="relative">
            ...
            <button 
              ...
              className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-slate-900 transition-colors"
            >
          ...
          <button 
            disabled={isLoading}
            className={`w-full bg-slate-900 py-4 rounded-xl font-black italic text-white hover:bg-black transition-all active:scale-95 shadow-lg shadow-black/30 uppercase tracking-widest text-xs ${isLoading ? 'opacity-50 cursor-wait' : ''}`}
          >
        ...
        <div className="mt-8 text-center">
          <button 
            ...
            className="text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-slate-900 transition-colors"
          >
