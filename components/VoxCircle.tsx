...
const PostCard = React.memo(({ post, currentUser, isDarkMode, onLike, onCommentToggle, onCommentDelete, onDelete, isCommenting }: {
  ...
  return (
    <motion.div 
      ...
      className={`p-8 rounded-[2.5rem] border ${isDarkMode ? 'bg-zinc-900/30 border-white/5' : 'bg-white border-black/5 shadow-xl'}`}
    >
      <div className="flex justify-between items-start mb-6">
        <div className="flex gap-4">
          <div className="w-12 h-12 rounded-2xl bg-red-600/10 overflow-hidden border-2 border-red-600/20">
              <LazyLoadImage 
                src={`https://api.dicebear.com/9.x/adventurer/svg?seed=${post.username}&backgroundColor=f8fafc,f1f5f9&radius=20`} 
                alt="avatar" 
                effect="blur"
