...
const Dashboard: React.FC<{ isDarkMode: boolean; currentUser: User | null; onLogout: () => void }> = ({ isDarkMode, onLogout }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(statsRef, (docSnap) => { if (docSnap.exists()) setGlobalStats(docSnap.data() as any); });
    return () => unsubscribe();
  }, []);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => { setLiveVotes(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Vote[]); });
    return () => unsubscribe();
  }, []);
  ...
  useEffect(() => {
    if (role === 'ADMIN' && currentUser) {
      ...
      const unsubscribePostsCount = onSnapshot(collection(db, 'posts'), (snapshot) => { setLiveStats(prev => ({ ...prev, votesToday: snapshot.size })); });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => { setAdminQuizHistory(snapshot.docs.map(doc => doc.data())); });
    return () => unsubscribe();
  }, [currentUser, role]);
  ...
  useEffect(() => {
    if (role === 'ADMIN') {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => { setFeedbacks(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Feedback))); });
      return () => unsubscribe();
    }
  }, [role]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => { setMyPosts(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))); });
    return () => unsubscribe();
  }, [currentUser, role]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => { setAllPosts(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))); });
    return () => unsubscribe();
  }, [role]);
